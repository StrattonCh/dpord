...
run_sims <- function(sim_tbl, niter = 150000, nchains = 3, nburnin = 100000, thin = 5, max_clus = 8, max_attempts = 6){
  ...
  for(i in 1:nrow(sim_tbl)){
    # message
    message(paste0("Beginning sim ", i, " of ", nrow_sim_tbl))
    ...
    # run one sim
    onerun <- one_sim(
      mat = compas_data[[k_]][[sim_]]$Y,
      true_coords = compas_data[[k_]][[sim_]]$true_coords,
      k = k_, 
      sim = sim_, 
      niter = niter,
      nchains = nchains,
    ...
    saveRDS(
      onerun, 
      paste0("simulations/agg_summaries/", onerun, "_", i, ".rds")
    )
    out[[i]] <- onerun
  }
  return(out)
...
saveRDS(sims, "simulations/agg_summaries/sims.rds")
```
